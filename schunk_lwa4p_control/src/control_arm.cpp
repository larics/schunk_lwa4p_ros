...
/*bool
ControlArm::executeCartesianServiceCallback(schunk_lwa4p_control::CartesianPath::Request
&req, schunk_lwa4p_control::CartesianPath::Response &res)
{
    ...
    /*        geometry_msgs::Pose tmp_pose;
            tmp_pose.position = currentPose.position; mid_pose.orientation =
currentPose.orientation; tmp_pose.position.z = currentPose.position.z + i *
(target_pose.pose.position.z - currentPose.position.z)/num_points;
    ...
    ROS_INFO_NAMED("tutorial", "Visualizing plan  (Cartesian path) (%.2f%%
...
ControlArm::getClosestPointOnLine(geometry_msgs::Vector3 line_point,
  ...
  while (ros::ok) {
    ...
    try {
      ...
      if (magnetic_localization) {
        // TODO: Add to specific method or class to handle this, maybe create
        // ControlArm node as virtual class and this could be MagneticArm which
        // inherits that class
        tf::StampedTransform transform1;
        tf::StampedTransform transform2;
        tf::StampedTransform transform3;
        tf::StampedTransform powerline0_transform;
        tf::StampedTransform powerline1_transform;
        geometry_msgs::PoseStamped powerline0Pose;
        geometry_msgs::PoseStamped powerline1Pose;
        geometry_msgs::Quaternion poseQuaternion;

        std::string needed_frame = "base_link";
        std::string target_frame = "lwa4p_base_link";
        listener.lookupTransform(needed_frame, "power_line0_n", ros::Time(0),
                                 powerline0_transform);
        listener.lookupTransform(needed_frame, "power_line1_n", ros::Time(0),
                                 powerline1_transform);
        listener.lookupTransform(target_frame, "power_line0_n", ros::Time(0),
                                 transform1);
        listener.lookupTransform(target_frame, "power_line1_n", ros::Time(0),
                                 transform2);

        tf::Matrix3x3 rotation_matrix(transform1.getRotation()); // getBasis
        tf::Vector3 x_dir = rotation_matrix.getColumn(0);
        geometry_msgs::Vector3 x_dir_converted;
        geometry_msgs::Vector3 line1;
        geometry_msgs::Vector3 line2;
        geometry_msgs::Vector3 base;
        base.x = 0;
        base.y = 0;
        base.z = 0;
        x_dir_converted.x = x_dir.x();
        x_dir_converted.y = x_dir.y();
        x_dir_converted.z = x_dir.z();
        line1.x = transform1.getOrigin().x();
        line1.y = transform1.getOrigin().y();
        line1.z = transform1.getOrigin().z();
        line2.x = transform2.getOrigin().x();
        line2.y = transform2.getOrigin().y();
        line2.z = transform2.getOrigin().z();
        geometry_msgs::Vector3 close1;
        geometry_msgs::Vector3 close2;
        close1 = getClosestPointOnLine(line1, x_dir_converted, base);
        close2 = getClosestPointOnLine(line2, x_dir_converted, base);

        // Transform from last link (ee frame) to the end of the separator main
        // link of the separator
        listener.lookupTransform("lwa4p_link6", "separator_main", ros::Time(0),
                                 transform3);

        // ROS_INFO_STREAM("[world] x: " << (transform1.getOrigin().x() +
        // transform2.getOrigin().x()) / 2); ROS_INFO_STREAM("[world] y: " <<
        // transform1.getOrigin().y()); ROS_INFO_STREAM("[world] z: " <<
        ...
        final_translation.setZ((close1.z + close2.z) / 2 -
        ...
        broadcaster.sendTransform(tf::StampedTransform(
            final_transform, ros::Time::now(), target_frame, "goal_frame"));
        ...
        powerline0Pose.pose.orientation.x =
            powerline0_transform.getRotation().x();
        powerline0Pose.pose.orientation.y =
            powerline0_transform.getRotation().y();
        powerline0Pose.pose.orientation.z =
            powerline0_transform.getRotation().z();
        powerline0Pose.pose.orientation.w =
            powerline0_transform.getRotation().w();
        ...
        powerline1Pose.pose.orientation.x =
            powerline1_transform.getRotation().x();
        powerline1Pose.pose.orientation.y =
            powerline1_transform.getRotation().y();
        powerline1Pose.pose.orientation.z =
            powerline1_transform.getRotation().z();
        powerline1Pose.pose.orientation.w =
            powerline1_transform.getRotation().w();
    ...
    if (christmas_fair) {
      ...
      pose1.position.x = 0.474;
      pose1.position.y = 0.007;
      pose1.position.z = 1.04;
      pose1.orientation.x = 0.6799;
      pose1.orientation.y = 0.7330;
      pose1.orientation.z = 0.0146;
      pose1.orientation.w = 0.0085;

      pose2.position.x = 0.46;
      pose2.position.y = 0.0;
      pose2.position.z = 1.28;
      pose2.orientation.x = 0.6799;
      pose2.orientation.y = 0.7330;
      pose2.orientation.z = 0.0146;
      pose2.orientation.w = 0.0085;

      pose3.position.x = 0.227;
      pose3.position.y = 0.279;
      pose3.position.z = 1.280;
      pose3.orientation.x = 0.6799;
      pose3.orientation.y = 0.7330;
      pose3.orientation.z = 0.0146;
      pose3.orientation.w = 0.0085;

      pose4.position.x = 0.06;
      pose4.position.y = 0.46;
      pose4.position.z = 1.196;
      pose4.orientation.x = 0.6799;
      pose4.orientation.y = 0.7330;
      pose4.orientation.z = 0.0146;
      pose4.orientation.w = 0.0085;

      pose5.position.x = 0.069;
      pose5.position.y = 0.462;
      pose5.position.z = 0.98;
      pose5.orientation.x = 0.6799;
      pose5.orientation.y = 0.7330;
      pose5.orientation.z = 0.0146;
      pose5.orientation.w = 0.0085;

      std::vector<geometry_msgs::Pose> executionStreamPoses = {
          pose2, pose1, pose2, pose3, pose4, pose5, pose4};
